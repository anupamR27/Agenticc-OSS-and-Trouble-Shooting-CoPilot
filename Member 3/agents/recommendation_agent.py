...
def _print_recommendation_report(rca_result: dict[str, Any]) -> None:
    """Print a professional, human-readable recommendation report."""
    result = get_recommendations(rca_result)
    severity = str(rca_result.get("severity", "Not provided"))

    print("=" * 50)
    print("Recommendation Agent Demo")
    print("=" * 25)
    print()
...
if __name__ == "__main__":
    sample_rca_inputs: list[dict[str, Any]] = [
        {
            "cell_id": "CELL_102",
    ...
    for sample_rca in sample_rca_inputs:
        _print_recommendation_report(sample_rca)

    print("Raw Function Output Example:")
    pprint(get_recommendations(sample_rca_inputs[0]), sort_dicts=False)
